...
model_name = './model/seg-model-distilBERT-finetuned'
    ...
    for i, label in enumerate(papers['is_eos']):
        ...
        for word_idx in word_ids:
            if word_idx is None:
                ...
                label_ids.append(
                    label[word_idx] if label_pos == 'all' else -100)
...
def punctuate(texts: List[str]) -> List[str]:
    ...
    for batch in dataloader:
        ...
        pred = torch.argmax(logits, dim=2).squeeze()
